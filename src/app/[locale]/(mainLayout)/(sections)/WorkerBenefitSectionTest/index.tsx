...
function WorkerBenefitSectionTest() {
  const settings = {
    customPaging: function (i: number) {
      return (
        <div
          onClick={_handleClickSwiper}
          className={`${currentIndex === i ? ' border-[#FCB713]' : 'scale-90 border-transparent opacity-70'}  group relative flex cursor-pointer items-center justify-center overflow-hidden rounded-lg border-[2px] bg-white transition hover:border-[#FCB713]/80`}
        >
          <ImageFallback
            src={listDataBenefit[i].img}
            alt={listDataBenefit[i].title}
            height={200}
            width={200}
            className={`w-full cursor-pointer select-none object-cover transition group-hover:scale-105 ${currentIndex === i && 'scale-105'} `}
          />
          <span className='absolute left-2 top-2 z-40 flex size-12 items-center justify-center rounded-full bg-black text-sm text-white md:size-8 '>{i < 9 ? `0${i + 1}` : i + 1}</span>
        </div>
      )
    },
    dots: true,
    ...
    <div className='slider-container'>
      <Slider {...settings}>
        {listDataBenefit?.map((item: any, index: number) => {
          return (
            <div key={item.uuid} className={`${currentIndex === index ? 'visible' : 'invisible'}`} onClick={_handleClickSwiper}>
...
export default WorkerBenefitSectionTest
